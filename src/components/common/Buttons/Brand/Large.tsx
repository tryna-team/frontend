...
interface BrandButtonLargeProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  children?: React.ReactNode;
}

export default function BrandButtonLarge({ children, className, ...props }: BrandButtonLargeProps) {
  return (
    <Button
      className={cn(
        "relative w-[358px] h-12 rounded-full overflow-hidden border-none",
        "font-[Pretendard] text-[15px] font-semibold text-white",
        "bg-white hover:bg-white px-15",
