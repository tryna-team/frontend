...
export const actionItemService = {
  ...
  create: (eventId: number | string, request: ActionItemSaveRequest) =>
    apiClient.post<ActionItemSaveResponseData>(ENDPOINTS.ACTION_ITEMS.BY_EVENT(eventId), request),

  /** E106 저장된 준비/실행 항목의 완료 상태를 변경한다. */
  updateStatus: (actionItemId: number | string, request: ActionItemStatusUpdateRequest) =>
  ...
  getByEvent: (eventId: number | string, occurrenceDate?: string) =>
    apiClient.get<EventActionItemResponse>(
      ENDPOINTS.ACTION_ITEMS.BY_EVENT(eventId),
      occurrenceDate ? { params: { occurrenceDate } } : undefined,
    ),

  /** F104 선택 날짜에 표시할 시간형 실행 항목을 조회한다. */
  getTimedByDate: (date: string) =>
    apiClient.get<TimedActionItemResponse>(ENDPOINTS.ACTION_ITEMS.CALENDAR_TIMED, {
      params: { date },
    }),
};
