...
function CalendarGrid({
  ...
  const headerRef = useRef<HTMLDivElement>(null); // 상단 헤더 (버튼/월/요일)
  ...
  // 날짜 칸 높이(--day-row-h)를 계산해서 각 패널에 넣어준다. 요일 행은 CalendarHeader로
  // 이동했으므로 이전·현재·다음 패널 모두 전체 높이를 주 수로 나눈다.
  useEffect(() => {
    ...
    const updateRowHeights = () => {
      ...
      const setRowHeight = (el: HTMLElement | null, weeks: number) => {
        if (!el || weeks <= 0) {
          return;
        }
        const rowHeight = available / weeks;
        if (rowHeight > 0) {
    ...
    // FullCalendar 내부 DOM이 비동기로 갱신되는 경우에도 현재 패널의 행 높이를 다시 동기화한다.
  ...
  return (
    ...
    >
      <CalendarHeader
        ...
        onBack={onYearViewClick}
        onSearchClick={onSearchClick}
        onViewToggleClick={onViewToggleClick}
